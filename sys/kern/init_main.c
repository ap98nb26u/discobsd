...
//	printf("SP=%x, u=%x\n", sp, &u);
	...
	/*
	 * Seed the clock from the real-time clock if the machine has one,
	 * otherwise from the root filesystem's last-write time. inittodr()
	 * is machine-dependent (see each arch's machdep.c).
	 */
	inittodr(fs->fs_time);
	boottime = time;
